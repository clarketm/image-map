...
const space = (n, s) => new Array(Math.max(0, 10 + n - (s || "").length)).join(" ") + (s || "");

const bytes = b => `${b.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")} bytes`;
...
Promise.all([
  ...
  .then(([rawNew, rawOld, minNew, minOld, zipNew, zipOld]) => {
    console.log(`  Raw: ${space(14, bytes(rawNew).cyan)}       ${space(15, diff(rawNew, rawOld))}`);
    console.log(
